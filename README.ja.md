...
通知は任意の機能で、設定して有効にするまでは無効です。通知は常にいずれか1つのチャンネルから送信され、*通知チャンネル*サブメニュー（ラジオ選択、初期状態はTelegram）でチャンネルを切り替えられます。

**Telegram**では従来どおりの設定を行います。
...
承認済みの初期設定では、`away`、`lost`、`intruded`がオン、`unlocked`がオフです。通知自体は無効で開始します。`intruded`の写真撮影は初期状態でオンですが、写真を添付できるイベントは`intruded`だけで、ほかのイベントは常にテキストのみです。カメラへのアクセスが拒否された場合や撮影に失敗した場合、`intruded`通知もテキストのみで送信されます。一時的に作成された侵入検知写真は、送信の成否にかかわらず、送信を試みるたびに削除されます。

従来の`~/Library/Application Scripts/jp.sone.BLEUnlock/event`スクリプトも引き続き利用でき、選択中のチャンネルとは独立して4種類すべてのイベント引数を受け取ります。
...
以前のREADMEでは、ここにLINE NotifyとSnapshotUnlockerを使用するスクリプト例を掲載していました。LINE Notifyは終了したため、その過去のエンドポイントとサンプルは非対応であり、現在は動作しません。BLEUnlock内蔵の通知設定を使用するか、従来の`event`スクリプトから現在利用できる別のサービスに接続してください。
