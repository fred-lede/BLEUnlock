...
- **`SYNO.Chat.Post`** (`method=create`, `version=5`) sends photos: (1) login `SYNO.API.Auth` `version=6` with `enable_syno_token=yes`, `format=sid`, `session=Chat` at `https://<host>/webapi/auth.cgi` → `sid` + `synotoken`; (2) multipart upload to `entry.cgi` with query params `api/method/version/channel_id/_sid`, header `X-SYNO-TOKEN`, cookie `id=<sid>`, multipart fields `type=file`, `message=""`, `conn_id=""`, and the photo as the `file` part → `file_id`; (3) form POST `message=<caption>&file_id=<file_id>` to the same `entry.cgi` URL with `X-SYNO-TOKEN` → `{"success": true}`. (Reference: N4S4/synology-api `post_file_upload`, confirmed on DSM 7.3.2 + Chat Server 2.4.5-22148.)
